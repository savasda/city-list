...
import { GenericHooks } from 'src/app/directives/generic-hooks.directive';
...
export class ListViewComponent extends GenericHooks {
	...
	onEditCity(city: CSVRowInterface): void {
		const dialogRef = this.dialogService
			.open(new PolymorpheusComponent(EditCityDialogComponent, this.injector), {
				data: {
					city
				}
			})
	...
	identify(index: number, item: CSVRowInterface): string {
